...
uint8_t RtcChip::transfer(uint8_t data) {
    ...
    if (state == IDLE) {
        ...
        result = toBCD(getSecond());
        state = RD_MINUTES;
    }
    else if (state == RD_MINUTES) {
        result = toBCD(getMinute());
        state = RD_HOURS;
    }
    else if (state == RD_HOURS) {
        result = toBCD(getHour());
        state = RD_DAYS;
    }
        ...
        result = toBCD(getWeekday());
        state = RD_MONTHS;
    }
    else if (state == RD_MONTHS) {
        result = toBCD(getMonth());
        state = RD_YEARS;
    }
    else if (state == RD_YEARS) {
        result = toBCD(getYear());
    }
    else if (state == WR_SECONDS) {
        setSecond(fromBCD(data));
        state = WR_MINUTES;
    }
    else if (state == WR_MINUTES) {
        setMinute(fromBCD(data));
        state = WR_HOURS;
    }
    else if (state == WR_HOURS) {
        setHour(fromBCD(data));
        state = WR_DAYS;
    }
        ...
        setWeekday(fromBCD(data));
        state = WR_MONTHS;
    }
    else if (state == WR_MONTHS) {
        setMonth(fromBCD(data));
        state = WR_YEARS;
    }
    else if (state == WR_YEARS) {
        setYear(fromBCD(data));
    }
    ...
    for (int i = 0; i < CH_NUM; ++i) {
        ...
        if (channel.convend_pin == convend_pin) {
            if (channel.simulator.getLoadEnabled()) {
                float u_set_v = channel.isRemoteProgrammingEnabled() ? remap(channel.simulator.voltProgExt, 0, 0, 2.5, channel.u.max) : channel.remapAdcDataToVoltage(u_set);
