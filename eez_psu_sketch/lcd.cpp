...
using namespace eez::psu::gui::lcd;
...
static const uint16_t nativeDisplayWidth = 240;
static const uint16_t nativeDisplayHeight = 320;

#if DISPLAY_ORIENTATION == DISPLAY_ORIENTATION_PORTRAIT
static const uint16_t displayWidth = nativeDisplayWidth;
static const uint16_t displayHeight = nativeDisplayHeight;
#else
static const uint16_t displayWidth = nativeDisplayHeight;
static const uint16_t displayHeight = nativeDisplayWidth;
#endif
...
void init() {
	...
	g_buffer = new uint16_t[displayWidth * displayHeight];
#endif
	...
	sbi(P_CS, B_CS);
#endif // EEZ_PLATFORM_ARDUINO_DUE
	...
	if (g_onOffTransition) {
		...
		if (diff > CONF_LCD_ON_OFF_TRANSITION_TIME) {
			...
			if (!g_isOn) {
				...
				fillRect(0, 0, displayWidth - 1, displayHeight - 1);
			}
			updateBrightness();
		} else {
...
static void setXY(uint16_t x1_, uint16_t y1_, uint16_t x2_, uint16_t y2_) {
	...
	x = x1;
	y = y1;
#endif // EEZ_PLATFORM_SIMULATOR || EEZ_PLATFORM_STM32

#if defined(EEZ_PLATFORM_ARDUINO_DUE)
	#if DISPLAY_ORIENTATION == DISPLAY_ORIENTATION_LANDSCAPE
		swap(uint16_t, x1_, y1_);
		swap(uint16_t, x2_, y2_);
		y1_ = nativeDisplayHeight - 1 - y1_;
		y2_ = nativeDisplayHeight - 1 - y2_;
...
static void adjustColor(uint8_t &ch, uint8_t &cl) {
...
static void setPixel(uint16_t color) {
	...
	if (x >= 0 && x < displayWidth && y >= 0 && y < displayHeight) {
		*(g_buffer + y * displayWidth + x) = color;
	}
	...
	if (x >= 0 && x < displayWidth && y >= 0 && y < displayHeight) {
		*(g_buffer + x * displayHeight + displayHeight - 1 - y) = color;
	}
